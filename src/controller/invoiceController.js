...
const getInvoice = async (req, res, next) => {
    try {
        ...
        const foundAccounts = await fnpaccounts.find({ _id: { $in: fnpaccountIds } });
        const foundPatients = await patdetails.find({ _id: { $in: pataccountIds } });

        const combinedData = invoices.map(invoice => {
            const relatedAccount = foundAccounts.find(account => account._id.toString() === invoice.corporateId.toString());
            const relatedPatient = foundPatients.find(patient => patient._id.toString() === invoice.patientId.toString());
