  ...
  createFCM: async (req, res, next) => {
    try {
      ...
      if (isAdmin === 'patient') {
        ...
        const getFCM = await FCM.findOne({ user: _id });
        if (getFCM) {
        ...
        const { fcmToken } = req.body;
        const superAdminUser = await superAdmin.findOne({ _id });

        if (superAdminUser) {
          // Update the fcmToken for the found super-admin user
          superAdminUser.fcmToken = fcmToken;
